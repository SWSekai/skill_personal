...
每次新對話開始時的一站式初始化入口：拉取專案更新、同步 Skill（合併原 `/skill sync` 流程一）、恢復上次工作 context、顯示狀態總覽。
...
| 2. 同步 Skill | 執行 `sp-sync.sh`（原 `/skill sync` 流程一）→ sekai-workflow → .claude/skills/ |
