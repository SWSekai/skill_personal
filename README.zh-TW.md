...
| `/skill <new\|sync\|pack>` | 管理 Skill 環境本身 — 建立、與上游同步、打包交接 | Skill 維護 |
...
- **上游更新** — 有人改進 Skill → `/hello` 或 `/skill sync` 自動拉入你的專案
- **下游回流** — 你在本地改進 Skill → 若具通用性，`/skill sync` 會詢問是否推回上游（需明確同意；預設僅本地）
...
你的調整只留在本地。若判定改動足夠通用，可標記出來，`/skill sync` 會協助回流至上游。
...
貢獻 Skill 改動前，先執行 `/skill sync` 確保 diff 基於最新上游版本。
