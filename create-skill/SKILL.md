---
name: create-skill
description: "建立新 Skill：互動式定義 → 生成 SKILL.md + README.md → 註冊至索引 → 同步 skill_personal"
...
在 `.claude/skills/<name>/SKILL.md` 建立 Skill 定義檔：
...
Arguments: $ARGUMENTS (描述參數用法)
```

**格式規範：**
- frontmatter 必須包含 `name`、`description`、`allowed-tools`
- 步驟以 `### Step N:` 格式編號
- 每步之間用 `---` 分隔
...
在 `.claude/skills/<name>/README.md` 建立說明文件：
...
1. **Skills Overview 表格**：在 `| Skill | Command | Description |` 表格中，按字母順序插入新行
2. **詳細說明區段**：在 `## 詳細說明` 區段，按字母順序插入新 Skill 的說明區塊
3. **Directory Structure 樹狀圖**：在 `skill_personal/` 的樹狀結構中加入新資料夾
...
**通用 Skill（適用所有專案）：**
1. 複製 `SKILL.md` 和 `README.md` 至 `skill_personal/<name>/`
2. 更新 `skill_personal/manifest.json`：在 `skills` 陣列中加入新條目
3. 更新 `skill_personal/README.md`：表格 + 說明 + 樹狀圖
4. 在 `skill_personal/` 中 commit + push：
   ```bash
   cd skill_personal
   git add <name>/SKILL.md <name>/README.md manifest.json README.md
