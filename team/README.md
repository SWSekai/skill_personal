...
> **2026-04-24 新增**：`report --daily` 模式 — 每日 Teams 報告，輸出 `hanschen/report/YYMMDD_daily_report.md`；資料源含 whiteboard/decision 結案摘要 + TODO 變化 + modify_log + 使用者交接；board 結案 Step 3.5、decide 結案 Step 6.6、`/commit-push` Step 11 **自動呼叫更新**；`/hello` Step 3.4 跨日檢查未處理交接；新增 `references/daily-report.md` + `assets/daily-report-template.md`。
...
| `report --daily [date]` | **每日 Teams 報告**（完成／進行中／待辦／交接／決策五段，輸出 `hanschen/report/YYMMDD_daily_report.md`；2026-04-24 剔除「brief」用字） | 手動 + **自動**（board/decide 結案 + commit-push 結尾） |
