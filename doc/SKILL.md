...
model: opus
effort: medium
argument-hint: "<info|trace|report> [args...]"
...
| 子命令 | 用途 | 對應原 Skill |
|---|---|---|
| `/doc info [topic]` | 系統資訊查詢與文件管理 | sys-info |
| `/doc trace <field>` | 端到端資料流追蹤 | trace-flow |
| `/doc report [scope]` | 從修改紀錄生成工作報告 | report |

無參數時要求使用者指定子命令。

---

## A. `/doc info` — 系統資訊查詢與文件管理
...
1. 先回答問題
2. 整理為結構化文件，存至 `.local/docs/[topic_description].md`
3. 告知使用者新檔位置
...
| 用法 | 行為 |
|---|---|
| `/doc report` | 全量報告（全部修改紀錄） |
| `/doc report weekly` | 近 7 天週報 |
| `/doc report YYMMDD YYMMDD` | 指定期間 |
