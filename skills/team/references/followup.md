...
2. 於 `.hanschen/board/` 與 `.hanschen/decision/` 下搜尋匹配檔（**過濾 `CLOSED_*`**）
3. 0 筆 / 1 筆 / 多筆 → 各自分支（見第 2 節）
4. Read 目標檔 → 依類型分派至 §3 / §4 處理器
5. 實作或推進單項 → 完成後結案

### 作用域
- 僅限 `.hanschen/board/` 與 `.hanschen/decision/` 兩目錄
- 不含 `.local/docs/summary/`（summary 目錄已於 2026-04-22 廢棄；結案摘要改寫入 CLOSED_ 原檔末尾）
- 不含 `.local/docs/tech-note/`、`.hanschen/handoff/`（非互動檔）
...
3. 更新 `.hanschen/journal/PROJECT_JOURNAL.md` 索引欄（連結指向 `CLOSED_*_decision.md`）
