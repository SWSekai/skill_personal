...
| `/ask <field>` | フィールドをスタック越しにトレース（データ欠損リスク注釈付き）；2026-05-06 以降 `info` サブコマンドは削除、システム情報の問い合わせは通常会話で | 「この値どう流れてる？」系 |
| `/skm <new\|sync\|pack\|update>` | Skill 環境自体の管理 — 作成・上流同期・引き継ぎパッキング（2026-04-24 に `/skill` から改名、ビルトイン `/skills` との衝突を回避）| Skill メンテ |
...
- **上流の更新** — 誰かが Skill を改良したら `/hello` か `/skm sync` で自動取り込み
- **下流の逆流** — ローカルで改良 → 汎用性があれば `/skm sync` が上流への push を確認（明示オプトイン、既定はローカルのみ）
...
ローカル編集は上流へは流れません。汎用的だと判断した場合、フラグを立てて `/skm sync` 経由で上流に貢献できます。
...
Skill 変更を出す前に `/skm sync` を一度走らせ、最新の上流に対する diff を取ってください。
